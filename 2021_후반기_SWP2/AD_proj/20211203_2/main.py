...
def display_primitive_screen():
    ...
    msg_classic.set_msg(SCREEN, (400, 450))
    msg_timer.set_msg(SCREEN, (400, 550))
    msg_arcade.set_msg(SCREEN, (400, 650))

    draw_buttons()

# 버튼 그리기
def draw_buttons():
    global classic_button, timer_button, arcade_button

    classic_button = Block(YELLOW, Rect(400, 450, 500, 50))
    timer_button = Block(ORANGE, Rect(400, 550, 500, 50))
    arcade_button = Block(RED, Rect(400, 650, 500, 50))
...
def check_buttons(pos):
    global game_set_mode, game_classic, game_timer, game_arcade
...
def check_movement():
    ...
    if not game_arcade:
        ...
        if BALL.rect.centery > 770 and len(BLOCKS) > 0:
            heart -= 1
            BALL.rect.center = (PADDLE.rect.centerx, PADDLE.rect.centery - 30)
        elif BALL.rect.centery < 20 and len(BLOCKS) > 0:
            heart -= 1
            BALL.rect.center = (PADDLE.rect.centerx, PADDLE.rect.centery + 30)
...
def object_initialize():
    global game_start, game_set_mode, score, heart, curr_stage, time_limit, start_tick, elapsed_time, BLOCKS, BALL, PADDLE
