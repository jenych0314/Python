...
def Parenthesis_Index_Check(string):
    ...
    for s in string:
        if s == '(' or s == '{' or s == '[' or s == '「' or s == '<' or s == '≪' or s == '『':#지금 문자가 여는 괄호인 경우
            temp_dict.setdefault(index)#딕셔너리 키값(여는 괄호 위치)에 추가
            temp.append(index)
            arr.append(s)
        elif s == ')' or s == '}' or s == ']' or s == '」' or s == '>' or s == '≫' or s == '』':#지금 문자가 닫는 괄호인 경우
...
try:
    file_read = open(path + '\\\\' + filename,'r',encoding='utf-8')
    file_write = open(path + '\\\\' + filename[:-4] + '_수정후' + filename[-4:],'w',encoding='utf-8')

    while True:
        line = file_read.readline()
        #readline()은 \n, \r, \r\n 기준으로 읽히고 저 문자들이 line에 같이 저장됨.
        if not line: break
        if line == '\n' or line == '\r' or line == '\r\n':#기존에 있는 \n만 있는 문자열들 전부 사라짐.
            continue
...
def Line_Break_Handling(string):
    ...
    if Parenthesis_Match_Check(string) == True:#괄호가 있을 경우
        ...
        for s in string:
            if s == '\"':
                pass
            if s in close_expression or s in exception_handling:
                temp.append(index)
                pass
            index += 1
...
# C:\Users\jeony\OneDrive\바탕 화면\PythonWork\test text