...
def Parenthesis_Index_Check(string):
    ...
    for s in string:
        if s in open_parenthesis:#지금 문자가 여는 괄호인 경우
            temp_dict.setdefault(index)#딕셔너리 키값(여는 괄호 위치)에 추가
            temp.append(index)
            arr.append(s)
        elif s in close_parenthesis:#지금 문자가 닫는 괄호인 경우
            if arr[len(arr) - 1] == parenthesis[s]:#
    ...
    start_index = StartEnd[0][0]
    end_index = StartEnd[0][1]
    re_range = {}

    for i in range(len(temp_dict)):#괄호 내부에 다른 괄호가 있을 경우 제외
        if StartEnd[i][0] <= start_index or StartEnd[i][1] >= end_index:
            start_index = StartEnd[i][0]
            end_index = StartEnd[i][1]
